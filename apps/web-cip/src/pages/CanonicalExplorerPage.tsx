...
export function CanonicalExplorerPage() {
  ...
  const { data: opsData, isLoading: opsLoading, error: opsError } = useQuery({
    queryKey: ["canonical-operations"],
    queryFn: listCanonicalOperations,
  });

  const { data: opDetail, isLoading: detailLoading } = useQuery({
    queryKey: ["canonical-operation", selectedOp?.operationCode, selectedOp?.latestVersion],
    queryFn: () =>
      getCanonicalOperation(selectedOp!.operationCode, selectedOp!.latestVersion),
    enabled: !!selectedOp,
  });
