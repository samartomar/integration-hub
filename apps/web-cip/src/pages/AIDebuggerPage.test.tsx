...
describe("AIDebuggerPage smoke", () => {
  ...
  it("when AI toggle ON, enhanceWithAi true is passed to API", async () => {
    ...
    await userEvent.setup().click(screen.getByRole("button", { name: "Analyze Request" }));
    await waitFor(() => {
      expect(endpointsApi.analyzeDebugRequest).toHaveBeenCalledWith(
  ...
  it("AI Summary section renders when API returns aiSummary", async () => {
    vi.mocked(endpointsApi.analyzeDebugRequest).mockResolvedValue({
      ...
      findings: [{ severity: "ERROR", code: "X", title: "X", message: "Invalid date", field: "payload.date", suggestion: "Use YYYY-MM-DD" }],
      normalizedArtifacts: {},
      notes: [],
      aiSummary: "The date field must use YYYY-MM-DD format.",
      remediationPlan: [{ priority: 1, title: "Fix date", reason: "Invalid format", action: "Use 2025-03-06" }],
