...
import {
  ...
  getSandboxCanonicalOperation,
  analyzeDebugRequest,
  analyzeDebugFlowDraft,
  analyzeDebugSandboxResult,
...
const SAMPLE_FLOW_DRAFT: Record<string, unknown> = {
...
export function AIDebuggerPage() {
  ...
  const [flowDraftJson, setFlowDraftJson] = useState(
    () => JSON.stringify(SAMPLE_FLOW_DRAFT, null, 2)
  );
  ...
  const { data: opsData, isLoading: opsLoading, error: opsError } = useQuery({
    ...
    queryFn: listSandboxCanonicalOperations,
  });

  const { data: opDetail } = useQuery({
    queryKey: ["sandbox-canonical-operation", selectedOp?.operationCode, selectedOp?.latestVersion],
    queryFn: () =>
      getSandboxCanonicalOperation(selectedOp!.operationCode, selectedOp!.latestVersion),
    enabled: !!selectedOp,
  });
  ...
  const handleAnalyzeRequest = useCallback(async () => {
    ...
    try {
      ...
      const result = await analyzeDebugRequest({
